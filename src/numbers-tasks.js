...
function getRectangleArea(width, height) {
  const rectanglArea = width * height;
  return rectanglArea;
}
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  ...
  const distancePoints = Math.sqrt(differenceX ** 2 + differenceY ** 2);
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const cosCorner = scalarProduct / (lengthVector1 * lengthVector2);
  const radians = Math.acos(cosCorner);
  return radians;
}
...
function getParallelepipedDiagonal(a, b, c) {
  const lengthDiagonal = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(lengthDiagonal);
}
